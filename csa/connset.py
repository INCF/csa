...
class CSet (CSAObject):
    ...
    def makeValueSet(self, k):
        ...
        raise RuntimeError(
            "don't know how to return value set for this connection-set")
    ...
    def __iter__(self):
        ...
        if isFinite(self.mask()):
            ...
            raise RuntimeError(
                'attempt to retrieve iterator over infinite connection-set')
...
class FiniteISetMask (FiniteMask, IntervalSetMask):
    ...
    def iterator(self, low0, high0, low1, high1, state):
        if not self.isBoundedBy(low0, high0, low1, high1):
            ...
            return IntervalSetMask.iterator(
                self, low0, high0, low1, high1, state)
