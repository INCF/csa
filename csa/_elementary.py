...
class FullMask(cs.IntervalSetMask):
    ...
    def __call__(self, N0, N1=None):
        if N1 == None:
            N1 = N0
        return cs.FiniteISetMask(
            iset.IntervalSet((0, N0 - 1)), iset.IntervalSet((0, N1 - 1))
        )
...
class OneToOne(cs.Mask):
    ...
    def __init__(self):
        super().__init__()  # Properly call the parent constructor
        self.name = OneToOne.tag

        if not hasattr(CSAObject, "tag_map"):
            CSAObject.tag_map = {}  # Ensure tag_map is a dictionary

        CSAObject.tag_map[str(CSA) + OneToOne.tag] = (self, SINGLETON)
...
class SampleNRandomOperator(cs.Operator):
    ...
    def __mul__(self, other):
        assert isinstance(other, cs.Finite) and isinstance(
            other, cs.Mask
...
class SampleNRandomMask(cs.Finite, cs.Mask):
    ...
    def startIteration(self, state):
        ...
        if "partitions" in state:
            ...
            N = numpy.random.multinomial(self.N, numpy.array(sizes) / float(total))
            obj.N = N[state["selected"]]
            obj.mask = partitions[state["selected"]]
            assert isinstance(
                obj.mask, cs.FiniteISetMask
