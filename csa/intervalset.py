...
class IntervalSet (CSAObject):
    ...
    def coerce(s):
        ...
        if s:
            ...
            for i in s[1:]:
                assert lastLower < i[0] and lastUpper < i[
                    0], 'intervals overlap'
                if i[0] - lastUpper == 1:
                    lastUpper = i[1]
                else:
    ...
    def skipIntervals(self):
        if len(self.intervals) <= 1 or self.intervals[
                0][0] != self.intervals[0][1]:
...
class ComplementaryIntervalSet (IntervalSet):
    ...
    def max(self):
        raise RuntimeError(
            'the maximum of a ComplementaryIntervalSet is infinity')
